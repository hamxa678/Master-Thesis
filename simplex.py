...
class Simplex_CLASS:
    ...
    def rand_3d_fixed_T_octaves(self, shape, T, octaves=1, persistence=0.5, frequency=32):
        ...
        assert len(shape) == 2
        batch_size = len(T)
        noise = np.zeros((batch_size, *shape))
        # print(f'rand_3d_fixed_T_octaves :: noise {noise.shape}')
        y, x = [np.arange(0, end) for end in shape]
        amplitude = 1
        for i in range(batch_size):
            for _ in range(octaves):
                T_scaled = np.array([T[i] / frequency])
                noise[i] += amplitude * self.noise3array(x / frequency, y / frequency, T_scaled).squeeze(0)
                frequency /= 2
                amplitude *= persistence
